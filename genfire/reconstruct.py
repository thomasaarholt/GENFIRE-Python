...
if __name__ != "__main__":
    ...
    def reconstruct(
        ...
        if R_freeInd_complex:
            Rfree_complex_bybin = np.zeros(
                (np.shape(R_freeInd_complex)[2], numIterations), dtype=float
            )
        ...
        if R_freeInd_complex:
            outputs = {
                "reconstruction": initialObject,
                "errK": errK,
                "R_free_bybin": Rfree_complex_bybin,
        ...
        iterationNumsToChangeCutoff = np.round(
            np.linspace(
                1, numIterations, num=np.size(constraintEnforcementDelayIndicators)
            )
        )
        iterationNumsToChangeCutoff, uniqueIndices = np.unique(
            iterationNumsToChangeCutoff, return_index=True
        )
        iterationNumsToChangeCutoff = np.append(
            iterationNumsToChangeCutoff, 1e30
        )  # add an arbitrarily high number to the end that is an iteration number that won't be reached
        constraintEnforcementDelayIndicators = constraintEnforcementDelayIndicators[
            uniqueIndices
        ]
        currentCutoffNum = 0
        ###
        ...
        for iterationNum in range(
            1, numIterations + 1
        ):  # iterations are counted started from 1

            if (
                iterationNum == iterationNumsToChangeCutoff[currentCutoffNum]
                ...
                constraintInd_complex = (
                    constraintIndicators > relevantCutoff
                ) * measuredK != 0
            ...
            errK[iterationNum - 1] = np.sum(
                abs(np.abs(fftObject[errInd]) - np.abs(measuredK[errInd]))
            ) / np.sum(
                abs(measuredK[errInd])
            )  # monitor error
            if verbose:
                print(
                    "Iteration number: {0}/{1}           error = {2:0.5f}".format(
                        iterationNum, numIterations, errK[iterationNum - 1]
                    )
                )
            ...
            if R_freeInd_complex:
                ...
                for shellNum in range(0, np.shape(R_freeInd_complex)[2]):
                    ...
                    Rfree_numerator = np.sum(
                        abs(np.abs(fftObject[tmpIndX, tmpIndY, tmpIndZ]) - tmpVals)
                    )
                    Rfree_denominator = np.sum(abs(tmpVals))
                    total_Rfree_error += Rfree_numerator
                    total_Rfree_error_norm += Rfree_denominator
                    Rfree_complex_bybin[shellNum, iterationNum - 1] = (
                        Rfree_numerator / Rfree_denominator
                    )
                Rfree_complex_total[iterationNum - 1] = (
                    total_Rfree_error / total_Rfree_error_norm
                )
    ...
    def fillInFourierGrid(
        ...
        * fillInFourierGrid *
        ...
        ky, kx = np.meshgrid(
            np.arange(-n2, n2, 1, dtype=float), np.arange(-n2, n2, 1, dtype=float)
        )
        ...
        for projNum in range(0, numProjections):
            ...
            R = np.array(
                [
                    [
                        np.cos(psi) * np.cos(theta) * np.cos(phi)
                    ...
                    [
                        -np.sin(psi) * np.cos(theta) * np.cos(phi)
                        - np.cos(psi) * np.sin(phi),
                        -np.sin(psi) * np.cos(theta) * np.sin(phi)
        ...
        for Yshift in range(-shiftMax, shiftMax + 1):
            for Xshift in range(-shiftMax, shiftMax + 1):
                for Zshift in range(-shiftMax, shiftMax + 1):
                    ...
                    distances = (
                        np.abs(measuredX - tmpX) ** 2
                        + np.abs(measuredY - tmpY) ** 2
                        + np.abs(measuredZ - tmpZ) ** 2
                    )
                    tmpX += nc
                    tmpY += nc
                    tmpZ += nc
                    ...
                    ) & (distances <= (interpolationCutoffDistance ** 2))

                    masterInd = np.append(
                        masterInd,
                        np.ravel_multi_index(
                            (
                                tmpX[goodInd].astype(np.int64),
        ...
        masterDistances[masterDistances != 0] = (
            1 / masterDistances[masterDistances != 0]
        )
        ...
        # accumulate the sums
        vals_real = np.bincount(
            masterInd, weights=(masterDistances * np.real(masterVals))
        )
        vals_cx = np.bincount(
            masterInd, weights=(masterDistances * np.imag(masterVals))
        )
        ...
        if verbose:
            print(
                "Fourier grid assembled in {0:0.1f} seconds".format(time.time() - tic)
            )
        return measuredK

    def fillInFourierGrid_DFT(
        projections,
        angles,
        interpolationCutoffDistance,
        enforce_resolution_circle,
        verbose=True,
    ):
        """
        * fillInFourierGrid_DFT *
        ...
        try:
            artifical_error = (
                1 / 0
            )  # This loop uses broadcasting, which can use more memory, so there is a try/catch as a failsafe. However
            ...
            for proj_num in range(num_projections):
                ...
                CP = pointToPlaneClosest(
                    np.vstack(
                        (
                            K1.flatten(order="F")[[Dind]],
                            K2.flatten(order="F")[[Dind]],
                            K3.flatten(order="F")[[Dind]],
                        )
                    ).T,
                    normVECs[proj_num, :].T,
                    np.zeros(np.size(Dind), dtype=float),
                )
                CP_plane = np.dot(np.linalg.inv(rotMATs[:, :, proj_num]), CP.T)

                Gind = Dind[
                    0, (abs(CP_plane[0, :]) <= n1 / 2) & (abs(CP_plane[1, :]) <= n2 / 2)
                ]
                G_CP_plane = CP_plane[
                    :, (abs(CP_plane[0, :]) <= n1 / 2) & (abs(CP_plane[1, :]) <= n2 / 2)
                ]
                ...
                Fpoints = np.sum(
                    curr_proj[nonzero_ind]
                    * np.exp(
                        -1j
                        * 2
                        * PI
                        * (K10[nonzero_ind] * X / n1 + K20[nonzero_ind] * Y / n2)
                    ),
                    axis=1,
                )
                distances = D[Gind]
                ...
                for i in range(np.shape(G_CP_plane)[1]):
                    Fpoints[i] = np.sum(
                        ...
                        * np.exp(
                            ...
                            * PI
                            * (
                                K10_t * G_CP_plane[0, i] / n1
                                + K20_t * G_CP_plane[1, i] / n2
                            )
                        )
                    )
    ...
    def readMAT(filename):
        ...
        * readMAT *
        ...
        try:  # try to open the projections as a stack
            ...
            while (
                flag
            ):  # first count the number of projections so the array can be initialized
...
class ReconstructionParameters:
    ...
    def __init__(self):
        ...
        self.eulerAngles = ""
        self.support = ""
        self.resolutionExtensionSuppressionState = (
            1  # 1 for resolution extension/suppression, 2 for off, 3 for just extension
        )
    ...
    def checkParameters(self):  # verify file extensions are supported
        ...
        if projection_extension[
            1
        ] not in ReconstructionParameters._supportedFiletypes or not os.path.isfile(
            self.projections
        ):
            parametersAreGood = 0
        ...
        if angle_extension[
            1
        ] not in ReconstructionParameters._supportedAngleFiletypes or not os.path.isfile(
            self.eulerAngles
        ):
            parametersAreGood = 0
        ...
        if not self.useDefaultSupport:
            if (
                self.support != ""
            ):  # empty support filename is okay, as this will trigger generation of a default support
    ...
    def toggleDisplayFigure(
        self,
    ):  # whether or not to display figure during reconstruction
...
class GenfireReconstructor:
    ...
    def __init__(
        ...
        support="",
        initialObject=None,
        resultsFilename=os.path.join(os.getcwd(), "results.mrc"),
    ...
    def printParams(self):
        ...
        print(
            "resolutionExtensionSuppressionState = {}".format(
                self.params.resolutionExtensionSuppressionState
            )
        )
        print("numIterations = {}".format(self.params.numIterations))
        print("oversamplingRatio = {}".format(self.params.oversamplingRatio))
        print(
            "interpolationCutoffDistance = {}".format(
        ...
        print(
            "enforceResolutionCircle = {}".format(self.params.enforceResolutionCircle)
        )
    ...
    def reconstruct(self):
        ...
        if isinstance(self.params.projections, str):
            ...
            )  # load projections into a 3D numpy array
        else:
        ...
        projections = np.pad(
            projections, ((padding, padding), (padding, padding), (0, 0)), "constant"
        )
        ...
        if self.params.griddingMethod == "DFT":
            measuredK = genfire.reconstruct.fillInFourierGrid_DFT(
                projections,
                eulerAngles,
                self.params.interpolationCutoffDistance,
            ...
            measuredK = genfire.reconstruct.fillInFourierGrid(
                projections,
                eulerAngles,
                self.params.interpolationCutoffDistance,
                self.params.enforceResolutionCircle,
        ...
        if self.params.calculateRfree:
            ...
            R_freeInd_complexZ = []
            R_freeVals_complex = []
            shell_thickness_pixels = (
                1  # pixel thickness of an individual shell of Rfree points
            )
            numberOfBins = int(
                round(dims[0] / 2 / shell_thickness_pixels)
            )  # number of frequency bins. Rfree will be tracked within each shell separately
            ...
            for shellNum in range(0, numberOfBins):
                ...
                measuredPointInd_complex = np.where(
                    (measuredK[:, :, 0 : (np.shape(measuredK)[-1] // 2 + 1)] != 0)
                    & (K_indicesSmall >= (spatialFrequencyForRfree[shellNum]))
                    & (K_indicesSmall <= (spatialFrequencyForRfree[shellNum + 1]))
                )

                # randomly shuffle
                shuffledPoints = np.random.permutation(
                    np.shape(measuredPointInd_complex)[1]
                )
                measuredPointInd_complex = (
                    measuredPointInd_complex[0][shuffledPoints],
                    measuredPointInd_complex[1][shuffledPoints],
                    measuredPointInd_complex[2][shuffledPoints],
                )

                # determine how many values to take
                cutoffInd_complex = np.floor(
                    np.shape(measuredPointInd_complex)[1] * percentValuesForRfree
                ).astype(int)

                # collect the Rfree values and coordinates
                R_freeInd_complexX.append(
                    measuredPointInd_complex[0][:cutoffInd_complex]
                )
                R_freeInd_complexY.append(
                    measuredPointInd_complex[1][:cutoffInd_complex]
                )
                R_freeInd_complexZ.append(
                    measuredPointInd_complex[2][:cutoffInd_complex]
                )
                R_freeVals_complex.append(
                    measuredK[
                        R_freeInd_complexX[shellNum],
                        R_freeInd_complexY[shellNum],
                        R_freeInd_complexZ[shellNum],
                    ]
                )
        ...
        if (
            self.params.resolutionExtensionSuppressionState == 1
        ):  # resolution extension/suppression
            constraintEnforcementDelayIndicators = np.array(
                np.concatenate(
                    (np.arange(0.95, -0.25, -0.15), np.arange(-0.15, 0.95, 0.1)), axis=0
                )
            )
        elif (
            self.params.resolutionExtensionSuppressionState == 2
        ):  # no resolution extension or suppression
            constraintEnforcementDelayIndicators = np.array([-999, -999, -999, -999])
        elif (
            self.params.resolutionExtensionSuppressionState == 3
        ):  # resolution extension only
            constraintEnforcementDelayIndicators = np.concatenate(
                (np.arange(0.95, -0.15, -0.15), [-0.15, -0.15, -0.15])
            )
        else:
            if self.params.verbose:
                print(
        ...
        ncBig = paddedDim // 2
        n2 = dims[0] // 2
        results["reconstruction"] = results["reconstruction"][
            ncBig - n2 : ncBig + n2, ncBig - n2 : ncBig + n2, ncBig - n2 : ncBig + n2
        ]
        return results
